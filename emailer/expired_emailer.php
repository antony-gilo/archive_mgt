 ...
 include('../connection.php');
	...
	$mail  = new PHPMailer();
				...
				//$body = "Your Password is changed to " ;
				 ...
				 //$query = 'SELECT COUNT(*)number FROM  inspection_register where date_due >CURDATE() AND date_due < CURDATE()+7';
				 $query = 'SELECT COUNT(*)number FROM  inspection_register where date_due<CURDATE() and notification=1';
	...
	$mail->Subject = "INSPECTION EXPIRED AFTER 14 DAYS";
	...
	$mail->MsgHTML(file_get_contents('http://192.168.3.43:81/admin_app/emailer/expired.php'));

	// $mail->MsgHTML('Hello This is a test');
	...
	$mail->AddAddress('admin@thetalgroup.com');
					//<========= for testing purposes before sending after a debug//======================>
	// $mail->AddAddress('antony.gilo@makupatransit.com');
...
// end get emails
			...
			//  $mail->AddCC('info@boss-freight.com');
			 ...
			 //===========> THIS WILL STOP NELSON FROM RECEIVING THIS EMAIL <======================
					//   $mail->AddCC('nelson.omukhango@th2g.org');
